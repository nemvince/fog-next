...
export const hostsApi = {
  list: (page = 1, limit = 25) =>
    api.get<{ data: Host[]; total: number }>(`/api/v1/hosts?page=${page}&limit=${limit}`),
  get: (id: string) => api.get<Host>(`/api/v1/hosts/${id}`),
  create: (host: Partial<Host>) => api.post<Host>('/api/v1/hosts', host),
  update: (id: string, host: Partial<Host>) => api.put<Host>(`/api/v1/hosts/${id}`, host),
  delete: (id: string) => api.delete<void>(`/api/v1/hosts/${id}`),
  listMACs: (id: string) => api.get<{ data: HostMAC[] }>(`/api/v1/hosts/${id}/macs`),
  ...
  getInventory: (id: string) => api.get<Inventory>(`/api/v1/hosts/${id}/inventory`),
  getActiveTask: (id: string) => api.get<import('./client').Task | null>(`/api/v1/hosts/${id}/task`),
}
...
export const imagesApi = {
  list: (page = 1, limit = 25) =>
    api.get<{ data: Image[]; total: number }>(`/api/v1/images?page=${page}&limit=${limit}`),
  get: (id: string) => api.get<Image>(`/api/v1/images/${id}`),
  create: (img: Partial<Image>) => api.post<Image>('/api/v1/images', img),
  update: (id: string, img: Partial<Image>) => api.put<Image>(`/api/v1/images/${id}`, img),
...
export const tasksApi = {
  list: (page = 1, limit = 25) =>
    api.get<{ data: Task[]; total: number }>(`/api/v1/tasks?page=${page}&limit=${limit}`),
...
export const groupsApi = {
  ...
  update: (id: string, g: Partial<Group>) => api.put<Group>(`/api/v1/groups/${id}`, g),
  delete: (id: string) => api.delete<void>(`/api/v1/groups/${id}`),
  listMembers: (id: string) => api.get<{ data: Host[] }>(`/api/v1/groups/${id}/members`),
...
export const snapinsApi = {
  ...
  update: (id: string, s: Partial<Snapin>) => api.put<Snapin>(`/api/v1/snapins/${id}`, s),
...
export const storageApi = {
  ...
  createGroup: (g: Partial<StorageGroup>) => api.post<StorageGroup>('/api/v1/storage/groups', g),
...
export const usersApi = {
  ...
  create: (u: Partial<User> & { password: string }) => api.post<User>('/api/v1/users', u),
  update: (id: string, u: Partial<User>) => api.put<User>(`/api/v1/users/${id}`, u),
  delete: (id: string) => api.delete<void>(`/api/v1/users/${id}`),
  regenerateToken: (id: string) => api.post<{ token: string }>(`/api/v1/users/${id}/regenerate-token`),
}
...
export const settingsApi = {
  list: (category?: string) =>
    api.get<{ data: Setting[] }>(`/api/v1/settings${category ? `?category=${category}` : ''}`),
  set: (key: string, value: string) => api.put<void>(`/api/v1/settings/${key}`, { value }),
}

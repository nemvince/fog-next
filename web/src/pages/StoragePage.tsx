...
export function StoragePage() {
  ...
  const [nodeForm, setNodeForm] = useState({ name: '', host: '', path: '', isMaster: false })
  ...
  const { data: nodesData } = useQuery({
    ...
    queryFn: () => storageApi.listNodes(selectedGroup!),
  ...
  const createNode = useMutation({
    mutationFn: () => storageApi.createNode(selectedGroup!, nodeForm),
    onSuccess: () => {
    ...
    <div className="p-8">
      ...
      <div className="grid grid-cols-2 gap-4">
              ...
              {groups.map((g) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="font-medium text-sm text-gray-100">{g.name}</p>
                    {g.description && (
        ...
        <div>
          ...
          {!selectedGroup ? (
            <p className="text-gray-500 text-sm">Select a group to view nodes</p>
          ) : (nodesData?.data ?? []).length === 0 ? (
            <p className="text-gray-500 text-sm">No nodes in this group</p>
          ) : (
            <div className="flex flex-col gap-2">
              {(nodesData?.data ?? []).map((n) => (
                <div key={n.id} className="flex items-center gap-3 rounded-lg border border-gray-800 bg-gray-900 px-4 py-3">
                  <Server className="h-4 w-4 text-gray-400" />
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <p className="font-medium text-sm text-gray-100">{n.name}</p>
                      {n.isMaster && <Badge variant="default">master</Badge>}
                      {!n.isEnabled && <Badge variant="outline">disabled</Badge>}
                    </div>
                    <p className="text-xs text-gray-500">{n.host}{n.path}</p>
                  </div>
                  <Button
                    variant="ghost"
                    size="icon"
      ...
      <Dialog open={groupDialog} onOpenChange={setGroupDialog}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle className="text-lg font-semibold text-gray-100">Add Storage Group</DialogTitle>
          </DialogHeader>
          <form onSubmit={(e) => { e.preventDefault(); createGroup.mutate() }} className="flex flex-col gap-4">
            <Input label="Name" value={groupForm.name} onChange={(e) => setGroupForm({ ...groupForm, name: e.target.value })} required />
      ...
      <Dialog open={nodeDialog} onOpenChange={setNodeDialog}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle className="text-lg font-semibold text-gray-100">Add Storage Node</DialogTitle>
          </DialogHeader>
          <form onSubmit={(e) => { e.preventDefault(); createNode.mutate() }} className="flex flex-col gap-4">
            <Input label="Name" value={nodeForm.name} onChange={(e) => setNodeForm({ ...nodeForm, name: e.target.value })} required />
            <Input label="Hostname / IP" value={nodeForm.host} onChange={(e) => setNodeForm({ ...nodeForm, host: e.target.value })} required />
            <Input label="Storage path" value={nodeForm.path} onChange={(e) => setNodeForm({ ...nodeForm, path: e.target.value })} required />
            <label className="flex items-center gap-2 text-sm text-gray-300">
              <input type="checkbox" checked={nodeForm.isMaster} onChange={(e) => setNodeForm({ ...nodeForm, isMaster: e.target.checked })} />
              Master node
            </label>
            <div className="flex justify-end gap-2">
              <Button type="button" variant="outline" onClick={() => setNodeDialog(false)}>Cancel</Button>
              <Button type="submit" disabled={createNode.isPending}>Create</Button>
            </div>
          </form>
        </DialogContent>
      </Dialog>

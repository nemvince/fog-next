import { snapinsApi, type Snapin } from '@/api/client'
...
export function SnapinsPage() {
  ...
  async function handleUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const snapin = await snapinsApi.create({ name: file.name, description: '' })
      ...
      <div className="mb-6 flex items-center justify-between">
        ...
        <div>
          ...
          <Button onClick={() => fileRef.current?.click()} disabled={uploading}>
            <Upload className="h-4 w-4" /> {uploading ? 'Uploading…' : 'Upload Snapin'}
          </Button>
        </div>
      </div>

      <DataTable table={table} isLoading={isLoading} emptyMessage="No snapins uploaded" />
    </div>
  )
}

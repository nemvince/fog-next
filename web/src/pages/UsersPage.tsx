...
export function UsersPage() {
  ...
  const createMutation = useMutation({
    mutationFn: () =>
      usersApi.create({ username: form.username, password: form.password, role: form.role, email: form.email, isActive: true }),
    onSuccess: () => {
  ...
  const regenMutation = useMutation({
    ...
    onSuccess: (data) => {
      toast('Token regenerated', { description: data.token, variant: 'success' })
    },
    onError: (e: Error) => toast(e.message, { variant: 'destructive' }),
  })
  ...
  return (
    <div className="p-8">
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle className="text-lg font-semibold text-gray-100">Create User</DialogTitle>
          </DialogHeader>

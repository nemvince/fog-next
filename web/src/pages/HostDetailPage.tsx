import { hostsApi, type Host } from '@/api/client'
...
export function HostDetailPage() {
  ...
  const { data: macsData } = useQuery({
    ...
    queryFn: () => hostsApi.listMACs(id!),
    enabled: !!id && tab === 'macs',
  })

  const { data: inventory } = useQuery({
    queryKey: ['hosts', id, 'inventory'],
    queryFn: () => hostsApi.getInventory(id!),
  ...
  const updateMutation = useMutation({
    mutationFn: () => hostsApi.update(id!, form),
    onSuccess: () => {
  ...
  const addMacMutation = useMutation({
    mutationFn: () => hostsApi.addMAC(id!, newMac, macDesc),
    onSuccess: () => {
  ...
  const deleteMacMutation = useMutation({
    mutationFn: (macId: string) => hostsApi.deleteMAC(id!, macId),
    onSuccess: () => {
      void qc.invalidateQueries({ queryKey: ['hosts', id, 'macs'] })
    ...
    <div className="p-8 max-w-3xl">
      ...
      <div className="mb-6 flex items-center gap-4">
        ...
        <div className="ml-auto flex items-center gap-2">
          {host.lastContact ? (
            <Badge variant="success"><Wifi className="mr-1 h-3 w-3" />Online</Badge>
          ) : (
            <Badge variant="outline"><WifiOff className="mr-1 h-3 w-3" />Never seen</Badge>
          )}
      ...
      <div className="mb-6 flex gap-1 border-b border-gray-800">
        {(['info', 'macs', 'inventory'] as Tab[]).map((t) => (
          ...
          >
            {t === 'macs' ? 'MAC Addresses' : t.charAt(0).toUpperCase() + t.slice(1)}
          </button>
        ))}
      </div>
            ...
            {(['isEnabled', 'useAad', 'useWol'] as const).map((key) => (
              <label key={key} className="flex items-center gap-2 cursor-pointer select-none">
                ...
                <span className="text-sm text-gray-300">
                  {key === 'isEnabled' ? 'Enabled' : key === 'useAad' ? 'Use AAD' : 'Use WoL'}
                </span>
              </label>
            ))}
          </div>
      ...
      {tab === 'macs' && (
        <div className="flex flex-col gap-4">
          <div className="rounded-lg border border-gray-800 bg-gray-900 divide-y divide-gray-800">
            {macs.length === 0 && (
              <p className="p-4 text-sm text-gray-500">No MAC addresses registered.</p>
            )}
            {macs.map((m) => (
          ...
          {/* Add MAC form */}
          <form
            onSubmit={(e) => { e.preventDefault(); addMacMutation.mutate() }}
          ...
          >
            ...
            />
            <Button type="submit" disabled={addMacMutation.isPending} className="mb-0.5">
      ...
      {tab === 'inventory' && (
        <div className="rounded-lg border border-gray-800 bg-gray-900 divide-y divide-gray-800">
          {!inventory ? (
            <p className="p-4 text-sm text-gray-500">No inventory data collected yet.</p>
          ) : (
            [
              ...
              ['UUID', inventory.uuid],
              ['CPU', `${inventory.cpuModel} (${inventory.cpuCores} cores @ ${inventory.cpuFreqMhz} MHz)`],

...
const stateVariant = (state: string): 'default' | 'success' | 'warning' | 'destructive' | 'outline' => {
  switch (state) {
...
export function TasksPage() {
  ...
  const stateFilter = tab === 'active' ? 'active' : tab === 'queued' ? 'queued' : ''
  ...
  return (
    <div className="p-8">
      ...
      <DataTable table={table} isLoading={isLoading} emptyMessage={`No ${tab} tasks`} />
    </div>
  )
}

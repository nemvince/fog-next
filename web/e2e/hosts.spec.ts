...
test.describe("Hosts list", () => {
	...
	test("displays the hosts table with at least one column header", async ({
		page,
	}) => {
...
test.describe("Create host", () => {
	...
	test("creates a new host via the UI form", async ({ page }) => {
		...
		await page
			.getByRole("button", { name: /add host|new host|create/i })
			.click();
	...
	test("deletes the created host", async ({ page }) => {
		...
		await expect(
			page.getByRole("row", { name: new RegExp(TEST_HOST_NAME) }),
		).toHaveCount(0);
	});
});

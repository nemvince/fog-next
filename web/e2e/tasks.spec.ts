...
test.describe('Tasks list', () => {
  ...
  test('displays the tasks table with expected columns', async ({ page }) => {
    ...
    await expect(page.getByRole('columnheader', { name: /type|task/i })).toBeVisible();
...
test.describe('Create and cancel task', () => {
  test('creates a deploy task and then cancels it', async ({ page }) => {
    ...
    if (!hostId || !imageId) {
      test.skip(!hostId || !imageId, 'No hosts or images available — seed the database first.');
      return;
    }
    ...
    await page.goto('/tasks');
    await expect(page.getByTestId(`task-row-${taskId}`).or(
      page.getByRole('row').filter({ hasText: taskId.slice(0, 8) }),
    )).toBeVisible();

    // Cancel the task.
    const taskRow = page.getByRole('row').filter({ hasText: taskId.slice(0, 8) }).first();
    await taskRow.getByRole('button', { name: /cancel/i }).click();

    // Confirm cancellation if a dialog appears.
    const confirmBtn = page.getByRole('button', { name: /confirm|yes|cancel task/i });
    ...
    await expect(
      page.getByRole('row').filter({ hasText: taskId.slice(0, 8) }).getByText(/cancel/i),
    ).toBeVisible();
  });
});
...
test.describe('Task creation via UI', () => {
  test('opens the create-task dialog from the tasks page', async ({ page }) => {
    ...
    const newTaskBtn = page.getByRole('button', { name: /new task|create task|add task/i });

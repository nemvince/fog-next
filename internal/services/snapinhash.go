...
// and updates the database, replacing the legacy MD5-based approach.
type SnapinHash struct {
	cfg   *config.Config
	store store.Store
}

func NewSnapinHash(cfg *config.Config, st store.Store) *SnapinHash {
	return &SnapinHash{cfg, st}
}
...
func (s *SnapinHash) computeHashes(ctx context.Context) {
	snapins, err := s.store.Snapins().ListSnapins(ctx, store.Page{Limit: 10000})
	if err != nil {
	...
	for _, sn := range snapins {
		...
		if hash != sn.Hash {
			sn.Hash = hash
			if err := s.store.Snapins().UpdateSnapin(ctx, sn); err != nil {
